...
def builtinToolsAdvice() -> str:
    ...
    return (
        ...
        "fetches them when it first needs them."
        "\n\nIt cannot fetch what its package index does not describe, and it "
        "refreshes that index only when it has none at all. An Arduino15 folder "
        "left behind by an older Arduino install carries one that predates "
        "these tools, which is why they are reported missing rather than "
        "downloaded. Refreshing it is the fix, and it is the first command:"
        "\n\n    arduino-cli core update-index"
        f"\n    arduino-cli core install {core}"
        "\n\nRun both, in that order. The first still prints the errors above - "
        "it is repairing the very thing they are about - so ignore them and run "
        "the second, which fetches the tools and reports the core as already "
        "installed without downloading it again."
    )
